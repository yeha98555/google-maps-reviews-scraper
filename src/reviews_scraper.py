...
review_default_result: Dict[str, Any] = {
    ...
    "response_relative_date": "",  # the localized relative date
...
def extract_reviews_and_photos(text):
    # Regular expression pattern to extract numbers that may represent
    # reviews and photos
    pattern = r"\d+"
...
class GoogleMapsAPIScraper:
    ...
    def _parse_sort_by(self, sort_by: str) -> str:
        """Default to newest"""
    ...
    def _cut_response_text(self, text: str) -> str:
        ...
        if idx_first_div == -1:
            idx_first_div = 0
        match = regex.search("</div", text, flags=regex.REVERSE)
        if match:
    ...
    def _get_response_token(self, response_text: str) -> str:
        """Searches for token in response text using regex,
    ...
    def _get_request(
        ...
        query = (
            f"https://www.google.com/async/reviewSort?"
            f"authuser=0&hl={hl}&yv=3&cs=1&async=feature_id:{feature_id},"
            f"review_source:All%20reviews,sort_by:{sort_by_id},"
            f"is_owner:false,filter_text:,associated_topic:,"
            f"next_page_token:{token},_pms:s,_fmt:pc"
        )
    ...
    def _parse_review_text(self, text_block) -> str:
        ...
        for e, s in zip(text_block.contents, text_block.stripped_strings):
            if isinstance(e, Tag) and e.has_attr("class"):
                break
            text += s + " "
    ...
    def _handle_place_exception(self, response_text, name, n):
        # Error log
        tb = re.sub(r"\s", " ", traceback.format_exc())  # Corrected
        ...
        print(new_var)
        with open(new_var, "w", encoding="utf-8") as f:
            ...
            if result["response_relative_date"]:
                try:
                    result["response_text_date"] = parse_relative_date(
                        result["response_relative_date"],
                        result["retrieval_date"],
                    )
                except Exception:
                    print(traceback.format_exc())
    ...
    def scrape_reviews(
        ...
        """Scrape specified amount of reviews of a place, appending
        results in csv"""
            ...
            while n > 0:
                ...
                try:
                    ...
                    (
                        ...
                        review_count,
                        next_token,
                    ) = self._get_request(  # type: ignore[misc]
                        feature_id,
                        hl=hl,
    ...
    def scrape_place(
        ...
        _, response_soup, _, _, _ = self._get_request(  # type: ignore[misc]
            feature_id,
            hl=hl,
        )
